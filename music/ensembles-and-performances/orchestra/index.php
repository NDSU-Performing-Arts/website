<?php 
//Edit the following:
$config['page-title'] = "USO | Challey School of Music";
$config['page-description'] = "";
$config['page-realm'] = "CSOM"; //Choose DPA, DTA, CSOM

//Leave the following alone because it's magic:
$config['page'] = $_SERVER['REQUEST_URI'];
$config['path'] = $_SERVER['DOCUMENT_ROOT']."/";
include($config['path'].'resources/header.php');
?>

<section id=""><div class="container"><!-- InstanceBeginEditable name="body" -->

<!--	body content -->
<section>
	<header>
		<h1>NDSU Orchestra Program</h1>
	</header>
	
	<article>
		<p><strong><em>University Symphony Orchestra</em> (USO) Full Symphony Orchestra MUSC 304 0-1 (#4583) 6:30-8:00pm Tuesday.<br />
		Audition Required</strong></p>

		<p><strong><em>String Chamber Ensemble</em> (SCE) – Strings Only MUSC 316 0-1 (#15404) 6:30-8:00pm Thursday <br />
		Audition Required</strong></p>

		<p><strong>Separate courses –1 credit each. May be repeated for credit. </strong></p>

		<p><u><strong>The University Symphony Orchestra</strong></u> will rehearse and perform standard classical, traditional, contemporary orchestral and chamber music repertoire. <br />
		This ensemble will provide members the opportunity to rehearse and perform orchestral literature at a high and challenging level.</p>

		<p><u><strong>The String Chamber Ensemble (Strings Only)</strong></u> will focus on developing technical skills and ensemble playing though string orchestra repertoire. Membership in this ensemble will provide string players the opportunity to rehearse and perform in an ensemble setting which – while still be committed to excellence - will be less arduous and less stressful than in the USO.</p>

		<p><strong>All String Students and *Community Members selected to play in the USO through audition are Strongly Encouraged <u>to participate in Both the USO AND the SCE.</u></strong></p>

		<h2>Audition requirements:</h2>
		<div class="alert alert-warning">
		<h3>SCE</h3>
		<p><strong>MUSC 316 0-1 (#15404) Rehearsals <u>6:30-8:00pm Thursday Night</u></strong></p>
		<p>There will be one excerpt chosen by the player. This excerpt should demonstrate the skill level of the player and can be either lyrical or technical. Excerpt should be @ 2 minutes maximum in length. Aspiring ensemble members need to have string playing experience – but this ensemble will be less rigorous than the USO and will emphasize and promote playing development and improvement through quality string orchestra literature. </p>
		</div>

		<div class="alert alert-warning">
		<h3>USO</h3>
				<p><strong>MUSC 304 -01 (#4583) Rehearsals 6:30-8:00pm Tuesday Night.</strong></p>
		<div class="alert alert-success">
		<h4>Strings</h4>

		<p>Participation in <strong>University Symphony Orchestra</strong> is through <strong>audition for all instruments</strong>. Players seeking to play in the <strong>USO</strong> will be required to audition at a generally higher level. It should be understood that not all who audition will make it into the USO. </p>

		<p>There will be a required etude/excerpt+ as well as a piece/excerpt chosen by the student. Total audition length should be @3-4 minutes (1 ½ to 2 minutes for each excerpt).<br />
		<u>Required</u> String Audition Music will be available by June 1st on the School of Music Website and via email from Johnson. </p>

		<p>Auditions will take place the weekend and Monday and Tuesday before classes begin for String players – August 18/19/20/21/22.</p>
		
		<p><strong>
		See required excerpts below (Mozart/violin or Bach/viola/cello/bass)<br />
		Scale(s) as indicated below.
		~2:00 excerpt or piece of your own choosing. </strong></p>
		
		<p>This is a "competitive" audition in the sense that not everyone who auditions will make the ensemble. If you aren't selected for the USO this fall - please make sure you stay engaged (be in the SCE) and audition again in the spring.</p>
		<p><strong>If you are successful and selected to be in the USO - you can also be and are encouraged to be in the SCE too. In other words - you can be in both ensembles. </strong></p>
 
		<p><a href="https://drive.google.com/drive/folders/1GY66uJvplRJ7_PG0c2BNT-vd1n48nx37?usp=sharing" class="btn btn- btn-ndsu-green">USO Required Audition excerpts and scales</a></p>

		<p>If Links don't work for you - please <a href="/performingarts/music/about/people/faculty/sigurd_johnson.php">contact me via email</a> and I will send the appropriate music and info to you.</p>
		
		<h5>USO String Audition Excerpts and Scales Performance Guidelines</h5>

		<h6>Violin</h6>
		<p><a href="https://www.youtube.com/watch?v=x7xPIyePmNk">Mozart Violin Sonata K. 301</a><br />
		Performance Tempo Quarter note = 126</p>

		<h6>Viola, Cello, Bass</h6>
		<p><a href="https://www.youtube.com/watch?v=pdsyNwUoON0">Bach Brandenburg Concerto No. 3</a><br />
		Performance Tempo (cut time) Half note = 88</p>

		<h6>Scales</h6>
		<p>Follow bowing and articulation instructions on the individual instrument sheet.  Follow Tempos as indicated. </p>
		<p><strong>Auditions for Strings will take place Friday/Saturday/Sunday/Monday/Tuesday August 18 - 22 before classes begin. There will be a google sign up for audition times.</strong></p>

		
		</div>
		
		<div class="alert alert-success">
		<h4>WW/Brass/Percussion Participation in USO:</h4>
		<p>As has been the practice in the past, <strong>all wind and brass students seeking to play in the USO will audition for their studio teachers.</strong><br /> 
		Individual studio teachers will decide the repertoire and audition requirements for their students. <br />
		Music for Winds/Brass and Perc will be available from individual studio teachers. <br />
		Contact your studio teachers of the appropriate studio over the summer for rep requirements. <br />
		(Auditions for Wind/Brass/Percussion will take the same time you audition for the Wind Symphony for the Fall) </p>
		</div>
		</div>

		<p><strong>*Fargo/Moorhead Community Members who play string instruments are very welcome to participate in either or both orchestral ensembles. <br />
		Please contact Sigurd Johnson for requirements and further information about auditions and our season. </strong></p>



		
	
		<!--<ul>
			<li><a href="https://drive.google.com/file/d/14oRnTGxTpZxBVpjDj6YqqC_Kx8zpZepY/view">Flute Audition</a></li>
			
			<li><a href="https://drive.google.com/file/d/14ub2fopup0hMyL74juhXa9uCQqW0klI8/view">Oboe Audition</a></li>
			
			<li><a href="https://drive.google.com/file/d/16FqHqchMW1jTnzEMYYEB1a5Cl68IhW8l/view">Clarinet Audition</a></li>
			
			<li><a href="https://drive.google.com/file/d/155PRd0c9tKuYe3hPLnp7n2zl8Q4NGsD7/view">Bassoon Audition</a></li>
			
			<li><a href="https://drive.google.com/file/d/15PcJP3lIsedzTwqCgcTpa8tEpGJqTn6y/view">Trumpet Audition</a></li>
			
			<li><a href="https://drive.google.com/file/d/15GAWqomDa-P6S6OYwqDNnfzMzB5eXVne/view">Horn Audition</a></li>
			
			<li><a href="https://drive.google.com/file/d/15J_4Ozku_JsDWte2UmFKU95GMFVUMKZR/view">Trombone Audition</a></li>
			
			<li><a href="https://drive.google.com/file/d/15L7Z20n8qMQEM0lOYJjH1L147osBQsFn/view">Tuba Audition</a></li>
			<li><a href="http://bit.ly/USOStringAuditionFall18">Strings Audition</a></li>
			
	
		</ul>
		
		<p>If you have questions, please contact Dr. Warren Olfert at 701-231-7266 or at <a href="mailto:warren.olfert@ndsu.edu">warren.olfert@ndsu.edu</a>.</p>-->
		</article>
		</section>
<!-- InstanceEndEditable --></div></section>
<?php include($config['path'].'resources/footer.php')?>