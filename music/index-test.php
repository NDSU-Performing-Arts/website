...
<div class="col-lg-9 col-md-12">
	...
	// Read the JSON file 
	$json = file_get_contents('https://api.ntfg.net/v1/ndsu-calendar.php');
	...
	?>
		if(count(array_filter($today))>0){echo "<h4>Coming Up:</h4>";
					// echo "<table class=\"table table-hover lz-resp-table\"><thead><th>Time</th><th>Event</th><th>Venue</th></thead><tbody>";	
						foreach($today as $row){
							$start = new DateTime($row['start'], new DateTimeZone("UTC"));
							$start->setTimezone(new DateTimeZone($_COOKIE['HammerLocalTZ']));
							...
							$end->setTimezone(new DateTimeZone($_COOKIE['HammerLocalTZ']));
							$end->add(new DateInterval('PT1H'));
				
					// echo "<tr><td>".$start->format("m/d/y g:i a") . "</td><td><strong><a href=\"/".$hammer->getHS('short')."/schedule/".$ev->page."/modify/".$ev->prettyID($row['id'],$row['name'])."/\">". $row['name'] . "</a></strong></td><td>" . $venue->row['name']."</td></tr>";
					
					echo "<div class=\"row border-bottom mt-2\">";
					echo "<div class=\"col-md-2 text-center\">
						<h1 class=\"display-4\"><span class=\"badge bg-liszt\">".$start->format('d')."</span></h1>
						<h2>".$start->format('M')."</h2>
					</div>
					<div class=\"col-md-10\">
						<h3 class=\"\"><a href=\"/".$hammer->getHS('short')."/schedule/".$ev->page."/modify/".$ev->prettyID($row['id'],$row['name'])."/\">".$row['name']."</a></h3>
						<ul class=\"list-inline\">
							<li class=\"list-inline-item\"><i class=\"fass fa-calendar\" aria-hidden=\"true\"></i> ".$start->format('l')."</li>
							<li class=\"list-inline-item\"><i class=\"fass fa-clock\" aria-hidden=\"true\"></i> ".$start->format('g:i a')."</li>
							<li class=\"list-inline-item\"><i class=\"fa fa-location-arrow\" aria-hidden=\"true\"></i> ".$venue->row['name']."</li>
						</ul>
						<p></p>
					</div>";
					echo "</div>";
